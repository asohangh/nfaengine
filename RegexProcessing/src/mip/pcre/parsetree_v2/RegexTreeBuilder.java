 ...
 * @author hieutt
 * 
 * This class's main purpose is for creating PCRE parse tree from various input types
 * + support only single pcre pattern.
 * Supporting output:
 * + To screen
 * + dot File format.
 *  
...
public class RegexTreeBuilder {
	...
	public RegexTree build(String pattern) {
		this.rule = new PCREPattern(pattern);
		this.index = 0;
		Node node = this.parsePcre();
		RegexTree tree = new RegexTree(node);
		tree.setPCREPattern(this.rule);
		return tree;
	}
