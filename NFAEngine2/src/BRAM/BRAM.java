...
public class BRAM {
    ...
    public LinkedList<BlockChar> listBlockChar; // need for routing from BRAM to State block
    // indentify each BRAM
    ...
    /**
     *  note:
     *      +, all engine must use modifier i or not.
     */
    ...
    System.out.print(this.listBlockChar.get(i).value + "[" + this.listBlockChar.get(i).id + "] ");
    }

    for (int i = 0; i < this.listBlockChar.size(); i++) {
    BlockChar bChar = this.listBlockChar.get(i);
    ...
    public void buildCOE() {
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(this._outputFolder + "BRAM_" + ID + ".coe"));
            bw.write(";");
    ...
    public void buildHDL() {
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(new File(this._outputFolder + "BRAM_" + this.ID + ".v")));
            ...
            bw.write("\n//BRAM declare \n");
            bw.write("\tbram_entity_" + this.ID + " ram (.addr(char),\n\t\t.clk(clk),\n\t\t.dout(q_out),\n\t\t.en(en));\n");
            //end of bram
            //declare engine
            int index = 0;
            for (int i = 0; i < this.engineList.size(); i++) {
    ...
    public void buildXCO() {
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(new File(this._outputFolder + "bram_entity_" + this.ID + ".xco")));
    ...
    public void buildCORE_RAM_HDL() {
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(new File(this._outputFolder + "bram_entity_" + this.ID + ".v")));
    ...
    /**
     * this function in charge of print list block char and coressponding block state
     */
