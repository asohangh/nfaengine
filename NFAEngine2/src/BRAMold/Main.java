...
public class Main {
    ...
    public static void buildfromfile(String name) throws IOException{
        ...
        BRAM btemp = new BRAM(0);
        String folders = btemp._outputFolder + File.separator;
        Main.createTopEngineTogether(folders, bramlist);
    }
 ...
 /*  public static void createTopEngineTogether(String folder, LinkedList<ReEngine> engine)throws Exception{
   ...
   public static void doBuildInterfacer(String folder)throws Exception{
    ...
    static void outputResult(String folder,LinkedList<BRAM> bramlist) {
        String outFol = folder + File.separator + "result.mip";
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(new File(outFol)));
            int State = 0;
            int Char = 0;
            for(int t = 0; t < bramlist.size(); t++ ){
                BRAM tbram= bramlist.get(t);
                int brState = 0;
                int brChar = 0;
                bw.write("BRAM " + t + "\n");
                bw.write("ListEngine: \n" );
                for(int i = 0; i < tbram.engineList.size(); i++){
                    ReEngine tre = tbram.engineList.get(i);
                    bw.write(tre.rule.toString() + "\n");

                    brState += tre.listBlockState.size();
                    brChar += tre.listBlockChar.size();
                }
                bw.write("\n\tTotal: \n");
                bw.write("\t BlockState: " + brState + "\n");
                bw.write("\t BlockChar : " + brChar + "\n");
            ...
            bw.write("\n\nTotal:\n");
            bw.write("\t BlockState: " + State + "\n");
            bw.write("\t BlockChar : " + Char + "\n");
